    ...
    <el-card shadow="hover">
      <template #header>
        <el-row :gutter="10" class="mb8">
          ...
          <el-col :span="1.5">
            <el-button v-hasPermi="['system:dict:edit']" type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()">
              修改
            </el-button>
          </el-col>
          <el-col :span="1.5">
      ...
      <el-table v-loading="loading" :data="typeList" @selection-change="handleSelectionChange">
        ...
        <el-table-column v-if="false" label="模型名称" align="center" prop="modelName" />
...
import modalIo from './modelData_api';
...
const getList = () => {
  ...
  modalIo.modelViews({params:proxy?.addDateRange(queryParams.value, dateRange.value) }).then(res => {
...
const submitForm = () => {
  dictFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      await  modalIo.modelViewsAdd({data:form.value});
...
const handleRefreshCache = async () => {
    digestReqest({
          method: 'get',
          url: '/v1beta/app/dataapp-RrPjRpDX/endpoint/mode',
          params: {
            page: 1,
            page_size: 20,
          }}).then(resp => {
