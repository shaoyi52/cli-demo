...
router.use('', async function (req, res, next) {
  ...
  let dictSort = tool.getParams(req, 'dictSort', true);
  ...
  try {
    ...
    VALUES ("${list.cssClass}","${list.dictCode}","${list.dictLabel}","${list.dictSort}","${list.dictType}","${list.dictValue}","${list.isDefault}","${list.listClass}","${list.remark}")`;
    await db.query(sql);
  } catch (err) {
